# -*- coding: utf-8 -*-
__title__ = "Запись из помещения в отделку"
...
from pyrevit import forms, revit, output, script
...
def get_unique_line_styles(doc):
    lines = FEC(doc).OfCategory(BuiltInCategory.OST_Lines).WhereElementIsNotElementType().ToElements()
    
    # Собираем уникальные стили линий по имени
    unique_styles = {}
    for l in lines:
        try:
            style = l.LineStyle
            name = style.Name
            if name not in unique_styles:
...
from Autodesk.Revit.DB import Category, GraphicsStyle, GraphicsStyleType

def collect_all_graphics_styles(doc):
    """
    Собирает все доступные GraphicsStyle из всех категорий документа.
    """
    styles = []

    cat_mgr = doc.Settings.Categories
    for cat in cat_mgr:
        for subcat in cat.SubCategories:
            gs = subcat.GetGraphicsStyle(GraphicsStyleType.Projection)
            if gs and gs not in styles:
...
# styles_dict = get_unique_line_styles(doc)
...
def get_line_styles(doc):
    """
    Получает все стили линий (GraphicsStyle) из категории OST_Lines.
    """
    styles = []
    cat_lines = doc.Settings.Categories.get_Item(BuiltInCategory.OST_Lines)

    for subcat in cat_lines.SubCategories:
        gs = subcat.GetGraphicsStyle(GraphicsStyleType.Projection)
        if gs:
            styles.append(gs)
...
styles = sorted(get_line_styles(doc), key=lambda s: s.Name)
# for style in styles:
#     print("{}".format(lfy(style.Id, style.Name)))
...
if select:
    with Transaction(doc, "Удаление Стилей линей") as t:
        t.Start()
        for s in select:
            name = s.Name
            try:
                doc.Delete(s.Id)
                output.print_md("  - Удалили: **{}**".format(name))
            except Exception as e:
                output.print_md("  - ⚠ Ошибка при удалении {}: {}".format(lfy(s.Id),e))
