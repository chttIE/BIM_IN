...
def main():
    ...
    if nwex == []:
        ...
        msg = """Найдены виды в имени которых есть Navis. Как поступим?"""
        ops = ['Удалить все и сделать новый','Открыть существующий']
        cfgs = {'option1': { 'background': '0xFF55FF'}}
        options = forms.CommandSwitchWindow.show(ops,
            message=msg,
            config=cfgs,)
        if options == "Удалить все и сделать новый":
            ...
            try:
                ...
                t.RollBack()
                forms.alert("Error")
        elif options == "Открыть существующий":
            try:
                make_active(nwex[0])
            except:
                pass
