...
def create_local_model(sourceModelPath,destFilePath,overwrite = True):
  
    try:
        model_name = os.path.basename(sourceModelPath)
        new_path = os.path.join(destFilePath, model_name)
        print(new_path)
        modelpath = ModelPathUtils.ConvertUserVisiblePathToModelPath(sourceModelPath)
        app.CopyModel(modelpath,new_path,overwrite)
        print("- :white_heavy_check_mark: Локальная копия создана: **{}**".format(os.path.basename(sourceModelPath)))    
    except Exception as ex:
        print("Ошибка при попытке создание локальной копию: {}".format(str(ex)))
        return False


destFilePath = forms.pick_folder(title="Выбор папки для сохранения RVT", owner=None)

#main

sel_models = select_file_local()

for el in sel_models:
	create_local_model(el,destFilePath)
