...
def export_image(cur_doc, folder_for_save):
    ...
    return result_path
# --- main ---
path_sel_files = forms.pick_file(file_ext='rfa', multi_file=True)
if not path_sel_files:
    script.exit()

app.RegisterFailuresProcessor(GlobalFailureSwallower())
for pa in path_sel_files:
    folder = os.path.dirname(pa)
    output.print_md("## Файл {}".format(pa))

    fam_uidoc = __revit__.OpenAndActivateDocument(pa)  # type: ignore
    d = fam_uidoc.Document
    output.print_md("- Открыл {}".format(d.Title))

    if make_view_for_save(d, fam_uidoc):
        # print("OK")
        png_path = export_image(cur_doc=d, folder_for_save=folder)
        del_material(cur_doc=d)
        ok, info = save_fam(cur_doc=d, folder=folder)

        if ok:
            # если требуется отложенная замена — сначала уводим фокус, затем закрываем и только потом заменяем
            if info and "deferred_replace" in info:
                tmp_dir, tmp_path, target_path = info["deferred_replace"]

                if activate_any_project_doc():
                    d.Close(False)  # теперь закрывать безопасно
                    finalize_replace(tmp_dir, tmp_path, target_path)
                else:
                    output.print_md("! Не удалось активировать проектный документ. Замена отменена.")
                    # подчищаем временное
                    try:
                        if os.path.exists(tmp_path):
                            os.remove(tmp_path)
                ...
                # прямое сохранение — можно просто закрыть семейство
                if activate_any_project_doc():
                    d.Close(False)
                else:
                    output.print_md("! Не удалось активировать проектный документ. Семейство не закрываю.")
    output.print_md("___")
app.RegisterFailuresProcessor(OFFGlobalFailureSwallower())
output.print_md("### Программа завершена")